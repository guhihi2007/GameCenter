...
public class FileHttpResponseHandler extends AsyncHttpResponseHandler {
    ...
    private long networkSpeed;//平均速度
    ...
    protected void sendResponseMessage(HttpResponse response) {
        ...
        try {
            ...
            if (statusCode == 416) {
                ...
                Log.i(TAG, "Output file already exists. Skipping download.StatusCode is 416");
            ...
            if (file.exists() && totalSize == file.length()) {
                Log.i(TAG, "Output file already exists. Skipping download.");
                throw new FileAlreadyExistException("Output file already exists. Skipping download.");
            } else if (tempFile.exists()) {
                // response.addHeader("Range", "bytes=" + tempFile.length()
                // +
                // "-");
                Log.i(TAG, "yahooo: " + response.getEntity().getContentLength());
                previousFileSize = tempFile.length();

                Log.i(TAG, "File is not complete, download now.");
                Log.i(TAG, "File length:" + tempFile.length() + " totalSize:" + totalSize);
    ...
    public int copy(InputStream input, RandomAccessFile out) throws IOException {
        ...
        try {
            ...
            while (!interrupt) {
                ...
                // 每当下载了3M时判断下文件是否存在
                downloadedLength += n;
                if (downloadedLength > 3000000) {
                    if (!tempFile.exists() || !file.exists()) {
                        Log.i("DirErrorTAG", "下载文件不存在, 抛异常下载失败");
                        throw new IOException("Dir or File not exist");
                    }
                    downloadedLength = 0;
                }
                ...
                if (networkSpeed == 0) {
                    if (errorBlockTimePreviousTime > 0) {
                        ...
                        if (expireTime > TIME_OUT) {
                            Log.e(TAG, "networkSpeed=0;connection time out");
