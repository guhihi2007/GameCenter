 ...
 * 活动页面下载按钮监听
 */
public class ActivityButtonClickListener extends BaseOnclickListener {
    private InstallButton mBtn;
    private boolean showRedPoint = true;

    public ActivityButtonClickListener(Context context, GameBaseDetail game, InstallButton btn, String pageName) {
    ...
    private void downCtrl(View v, final String mPageName) {
        ...
        if (eventData != null)
            LogUtils.d("hhh", "ActivityButtonListener:" + eventData.getPos() + "==p2:" + eventData.getSubPos());
        if (mGame == null) {
            return;
        }
        ...
        switch (state) {
            ...
            case InstallState.ignore_upgrade:
                mGame.setDownloadFrom(Constant.FROM_ACTIVITY);
                mBtn.game.setDownloadFrom(Constant.FROM_ACTIVITY);
...
//            --客户端点击重试，但服务端无返回数据，则客户端不做任何处理，仅弹出toast提示：请求失败
            ...
            case InstallState.installComplete: {
                try {
                    if (mGame.getGameDomainBase().getIs_accepted() == 1) {
                        return;
                    }
                    if (!Constant.FROM_ACTIVITY.equals(mGame.getDownloadFrom())) {
                        if (!OpenAppUtil.openApp(mGame, mContext,getmPageName())) {
                            mBtn.setViewBy(mGame.getState(), mGame.getDownPercent());
                        }
                        return;
                    }
                    ...
                    ToastUtils.showToast(mContext, "领取异常");
                    e.printStackTrace();
                }
                break;
            }
    ...
    private void acceptPoint() {
        ...
        ToastUtils.showToast(mContext, "领取成功，+" + mGame.getDownloadPoint() + "积分");
        mGame.setAccepted(1);
        mBtn.game.setAccepted(1);
        mBtn.setViewBy(mGame.getState(), mGame.getDownPercent());
        FileDownloaders.update(mGame);
//        DownFileService.getInstance(mContext).updateById(mGame);

        SyncPointsBean bean = new SyncPointsBean();
        bean.setActivity_id(Integer.parseInt(MyApplication.application.activityId));
        bean.setActivity_name(MyApplication.application.activityName);
        bean.setGame_id(String.valueOf(mGame.getId()));
        bean.setGame_name(mGame.getName());
        bean.setPoints(mGame.getDownloadPoint());

        ArrayList<SyncPointsBean> list = SyncPointsUtil.getSyncPointsList();
        list.add(bean);
        int totalPoints = 0;
        ...
        EventBus.getDefault().post(new RefreshPointsEvent(new SignPointsNet(totalPoints)));
    ...
    private void acceptPointForLogin() {
        Map<String, String> params = new HashMap<>();
        params.put("game_id", String.valueOf(mGame.getId()));
        params.put("game_name", mGame.getName());
        params.put("activity_id", MyApplication.application.activityId);
        params.put("activity_name", MyApplication.application.activityName);
        params.put("point_style", String.valueOf(Constant.POINTS_DOWNLOAD));
        Net.instance().executePost(Host.HostType.GCENTER_HOST, Uri2.SIGN_POINTS, params, new WebCallBackToBean<BaseBean<SignPointsNet>>() {
            ...
            public void handle(BaseBean<SignPointsNet> baseBean) {
                ...
                if (status == 1) {
                    ...
                    ToastUtils.showToast(mContext, "领取成功，+" + info.getPoints() + "积分");
                    mGame.setAccepted(1);
                    mBtn.game.setAccepted(1);
                    mBtn.setViewBy(mGame.getState(), mGame.getDownPercent());
                    //这里数据库和缓存都得改
                    FileDownloaders.update(mGame);
//                    DownFileService.getInstance(mContext).updateById(mGame);
                    EventBus.getDefault().post(new RefreshPointsEvent(info));
                    if (!OpenAppUtil.openApp(mGame, mContext,getmPageName())) {
                        mBtn.setViewBy(mGame.getState(), mGame.getDownPercent());
                    }
                } else {
                    ToastUtils.showToast(mContext, baseBean.getMessage());
                }
