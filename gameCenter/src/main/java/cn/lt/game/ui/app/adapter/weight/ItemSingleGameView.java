...
public class ItemSingleGameView extends ItemView {
    ...
    public ItemSingleGameView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        initWeakView();
    }

    public ItemSingleGameView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initWeakView();
    }

    public ItemSingleGameView(Context context, BaseOnclickListener clickListener) {
        super(context);
        initWeakView();
    ...
    private void initWeakView() {
        new WeakView<ItemSingleGameView>(this) {
            @Override
    ...
    public void fillView() {
        try {
            ...
            if (mGame != null) {
                ...
                if (TextUtils.isEmpty(mGame.getReviews())) {
                    mTagTv.setVisibility(View.GONE);
                } else {
                    mTagTv.setVisibility(View.VISIBLE);
                    mTagTv.setText(mGame.getReviews().trim());
                }
                String downloadCount = IntegratedDataUtil.calculateCountsV4(Integer.valueOf(mGame.getDownCnt()));
                String gameSize = IntegratedDataUtil.calculateSizeMB(mGame.getPkgSize());
                ...
                ImageloaderUtil.loadRoundImage(mContext, mGame.getIconUrl(), mLogoIv);
                dealWithRankGold();
                //给按钮设置统计数据
        ...
        if (Constant.PAGE_RANK_HOT.equals(mPageName) || Constant.PAGE_RANK_NEWS.equals(mPageName) || Constant.PAGE_RANK_OFFLINE.equals(mPageName) || Constant.PAGE_RANK_ONLINE.equals(mPageName)) {
            ...
            switch (mPosition) {
                ...
                default:
                    ...
                    mRankNumberView.setText("" + (mPosition + 1));
            }
        }
    }
