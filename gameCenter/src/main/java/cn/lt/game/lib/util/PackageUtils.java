...
import cn.lt.game.lib.ListUtils;
...
import cn.lt.game.update.PlatUpdateInfo;
import cn.lt.game.update.PlatUpdateManager;

import static cn.lt.game.install.autoinstaller.AutoInstallerContext.getAccessibilityStatus;

/**
 * PackageUtils
 * <ul>
 * <strong>Install package</strong>
 * <li>{@link PackageUtils#installNormal(Context, String, GameBaseDetail, boolean, PlatUpdateInfo, String, StatisticsEventData)}</li>
...
public class PackageUtils {
     ...
     * <li>else see {@link #installNormal(Context, String, GameBaseDetail, boolean, PlatUpdateInfo, String, StatisticsEventData)}</li>
     * </ul>
     *
     * @param context
     * @param filePath
    ...
    public static final int install(Context context, String filePath) {
        ...
        return installNormal(context, filePath, null, false, null, Constant.MODE_SINGLE, null) ? INSTALL_SUCCEEDED : INSTALL_FAILED_INVALID_URI;
    }
     ...
     * @param game
     * @param isFromPlatInstall
     * @param mUpdateInfo
     * @param install_mode
     * @param eventData
     * @return whether apk exist
     */
    public static boolean installNormal(Context context, String filePath, GameBaseDetail game, boolean isFromPlatInstall, PlatUpdateInfo mUpdateInfo, String install_mode, StatisticsEventData eventData) {
        try {
            String installPkg = PreferencesUtils.getString(context, Constant.INSTALL_PKG, Constant.DEF_INSTALL_PKG);
            if (isFromPlatInstall) {
                DCStat.platUpdateEvent(ReportEvent.ACTION_PLATUPDATEINSTALLREQUEST, PlatUpdateManager.getPlatUpdateMode(context), null, PlatUpdateManager.getPlatDownloadType(context), PlatUpdateManager.getPlatInstallType(context), Constant.versionName, PlatUpdateManager.getTargetVersionCode(context), PlatUpdateManager.getPlatDownloadAction(context));
            } else {
                //监控安装完成情况
                InstalledEventLooper.getInstance().startInstall(game); //轮询监控
                DCStat.installRequest(game, install_mode, (canAutoInstall() ? "auto" : "normal"));
            }
            Intent i = new Intent(Intent.ACTION_VIEW);
            File file = new File(filePath);
            if (!file.exists() || !file.isFile() || file.length() <= 0) {
                //移除轮询器中的监控任务
                LogUtils.d("fff", "安装时，文件不存在，或包有问题，移除任务了");
//                InstalledEventLooper.getInstance().setTaskRunning(game.task);  //停止轮询任务
                return false;
            }
            //  com.tencent.android.qqdownloader（应用宝）    com.vmall.client(华为商城)  com.vivo.browser     com.huawei.hidisk    com.huawei.appmarket
            ...
            i.putExtra("caller_package", installPkg); //cn.goapk.market   com.google.launcher
            context.startActivity(i);
        } catch (Exception e) {
            LogUtils.e("fff", "正常安装抛异常了");
            e.printStackTrace();
        }
        return true;
    }
    ...
    public static int installSilent(Context context, String filePath, String pmParams) {
        ...
        if (commandResult.errorMsg.contains("root install fail, because be intercepted")) {
            return ROOT_INSTALL_FAIL_BECAUSE_BE_INTERCEPTED;
        }
        return INSTALL_FAILED_OTHER;
    }
     ...
     * uninstall fail if permission denied
     */
    public static final int ROOT_INSTALL_FAIL_BECAUSE_BE_INTERCEPTED = -9578;
     ...
     * @return
     */
    public static boolean canAutoInstall() {
        return getAccessibilityStatus() == AutoInstallerContext.STATUS_OPEN;
    }
}
